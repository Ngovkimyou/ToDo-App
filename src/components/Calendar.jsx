...
function Calendar({ selectedDate, onDaySelect, markedDates = [] }) {
  ...
  function hasTasksInMonth(monthIndex) {
    ...
    return markedDates.some((dateKey) => dateKey.startsWith(monthKey));
  }

  function hasTasksInYear(yearValue) {
    return markedDates.some((dateKey) =>
      dateKey.startsWith(`${yearValue}-`)
    );
  }
        ...
        {view === "days" && (
          <div className="calendar-day-grid">
            ...
            {getCalendarCells(year, month).map((cell, index) => {
              ...
              const hasTasks = markedDateSet.has(getLocalDateKey(cellDate));
        ...
        {view === "years" && (
          <div className="calendar-pick-grid">
            {yearRange.map((y) => (
              ...
              >
                {y}
                {hasTasksInYear(y) && <span className="calendar-day-dot" />}
              </button>
            ))}
          </div>
        )}
